...
def split_trips(V1):
    ...
    for trip_df in trip_dfs:
        ...
        # 拆分超过15分钟的部分
        trip_df['数据采集时间'] = pd.to_datetime(trip_df['数据采集时间'])
        time_diff = trip_df['数据采集时间'].diff()
...
def process_trip_data(new_trip_dfs):
    ...
    for trip_df in new_trip_dfs:
        ...
        # (1) 出行时间
        trip_start_time = trip_df['数据采集时间'].iloc[0]

        # (2) 行程时间
        trip_end_time = trip_df['数据采集时间'].iloc[-1]
...
#%%
TJ_Weather = pd.read_excel('11-pretrained_incremental_learning/TJ W.xls')
CD_Weather = pd.read_excel('11-pretrained_incremental_learning/CD W.xls')
SZ_Weather = pd.read_excel('11-pretrained_incremental_learning/SZ W.xls')
W=[TJ_Weather,SZ_Weather,SZ_Weather,CD_Weather,SZ_Weather ,TJ_Weather]
CND01Y22_A=[]
# 循环处理V1到V20的DataFrame并得到相应的combined_data
for i in range(1, 7):
    # 生成变量名
    v_name = f'V{i}'  # 生成V1到V20的变量名
    ...
    Weather = W[i-1]
    Weather.iloc[:, 0] = pd.to_datetime(Weather.iloc[:, 0], format="%d.%m.%Y %H:%M")
    combined_data = merge_weather_data(DF, Weather)
    ...
    combined_data['VIN'] = f'CND01Y22V{i-1}'
    ...
    CND01Y22_A.append(combined_data)


combined_df = pd.concat(CND01Y22_A, ignore_index=True)
A=combined_df .describe()
#%%
del V1,V2,V3,V4,V5,V6
#%%
HZ_Weather = pd.read_excel('11-pretrained_incremental_learning/HZ W.xls')
CC_Weather = pd.read_excel('11-pretrained_incremental_learning/CC W.xls')
SZ_Weather = pd.read_excel('11-pretrained_incremental_learning/SZ W.xls')
W=[SZ_Weather,CC_Weather ,SZ_Weather,SZ_Weather,HZ_Weather ,SZ_Weather,SZ_Weather]
...
for i in range(7, 14):
    ...
    CND01Y22_B.append(combined_data)


combined_df = pd.concat(CND01Y22_B, ignore_index=True)
A=combined_df .describe()
#%%
CND01Y22=CND01Y22_A+CND01Y22_B
combined_df = pd.concat(CND01Y22, ignore_index=True)
A=combined_df .describe()
#%%


with open('11-pretrained_incremental_learning/data/CND01Y22.pkl', 'wb') as f:
    pickle.dump(CND01Y22, f)
