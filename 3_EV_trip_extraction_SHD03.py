...
def process_trip_data(new_trip_dfs):
    ...
    for trip_df in new_trip_dfs:
        ...
        energy_consumption = np.abs(((sum_current * sum_voltage) * 30 / 1000 / 3600).sum())
...
#%%
power_list=[18,48.3,12,12.9,51.9,12,12,14.38,52.5,12.1,9.1,7.99,9.03]
type_list=["Sedan PHEV","SUV","Sedan PHEV","Sedan PHEV","SUV","Sedan PHEV","SUV PHEV"
           ,"Sedan PHEV",'Sedan',"Sedan PHEV","Sedan PHEV","Sedan PHEV","Sedan PHEV"]
weight_list=[1610,1710,1835,1600,1635,1835,1730,1690,1550,1730,1480,1540,1665]
...
for i in range(1, 14):
    ...
    grouped_dfs = [group for name, group in V_i.groupby('vin')]

    for j in range(len(grouped_dfs)):
        # 处理数据
        new_trip_dfs = split_trips(grouped_dfs[j])
        new_trip_dfs = [df for df in new_trip_dfs if len(df) >= 30]
        if not new_trip_dfs:  # 如果new_trip_dfs为空，跳过当前循环，执行下一个循环
            continue
        ...
        combined_data['电池能量'] = power_list[i-1]
        combined_data['车辆类型'] = type_list[i-1]
        combined_data['整备质量'] = weight_list[i-1]
        combined_data['地点'] = 'SH'

        combined_data['VIN'] = f'SHD03Y21M{i-1}V{j}'

        combined_data[["行程能耗", "行程时间", '行程距离', '行程平均速度', '当前SOC'
            , '当前累积行驶里程', '单体电池电压极差', '单体电池温度极差', '绝缘电阻值', '电池能量', '整备质量']] = \
            combined_data[["行程能耗", "行程时间", '行程距离', '行程平均速度', '当前SOC'
                , '当前累积行驶里程', '单体电池电压极差', '单体电池温度极差', '绝缘电阻值', '电池能量',
                           '整备质量']].astype(float)

        combined_data = combined_data[combined_data['行程能耗'] <= combined_data['电池能量']]

        SHD03Y21.append(combined_data)


combined_df = pd.concat(SHD03Y21, ignore_index=True)
...
with open('11-pretrained_incremental_learning/data/SHD03Y21.pkl', 'wb') as f:
    pickle.dump(SHD03Y21, f)
